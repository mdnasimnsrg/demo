...
const Login = () => {
  ...
  const {data, loading, error,success, postData } = usePost(API +'users/login', 'post', true);
        ...
        <div className = 'flex py-11 lg:py-0 w-full min-h-screen mx-auto box-border justify-center'>
          ...
          <div className = 'lg:bg-white box-border lg:w-6/12 lg:pl-20 lg:pr-12 flex items-center'>
            <div >
              ...
              <h3 className = 'text-2xl font-bold text-grey-800 text-center pt-4'>Login</h3>
              <p className = 'text-sm text-center text-grey-800 pt-2'>Welcome to Ethica Invest</p>
              <form onSubmit = { handleFormSubmit }>
                <ul>
                  <li className = 'pt-8 lg:hidden'>
                    <div className = "relative">
                      <input id = "email" name = "email" type = "text" className = { `peer h-10 w-full border-b border-grey placeholder-transparent focus:outline-none focus:border-rose-600 ${errorEmail?'outline-red':'outline-primary-600'}` } placeholder = '' onChange = { handleEmail } onBlur = { handleEmailBlur } value = { email } />
                  ...
                  <li className = 'pt-6 lg:hidden'>
                    <div className = "relative">
                      <input id = "password" name = "password"  onChange = { handlePasswordChange } value = { password }type = { showPassword ? 'text' : 'password' } className = "peer h-10 w-full border-b border-grey placeholder-transparent focus:outline-none focus:border-rose-600 outline-primary-600" placeholder = '' />
                  ...
                  <li><div className = 'text-right py-4'><Link className = 'text-xs text-primary font-bold' to = "/forgot-password">Forgot Password</Link></div></li>
                  <li>
                    { error && <p className = 'text-xs text-red pb-1'>Error: { error == 'Invalid body'?'Password must contain at least 8 character(s)' : error }</p> }
                    <button type = 'submit' className = { `block w-full text-white rounded-3xl lg:rounded-md px-4 py-2.5 text-sm ${isEmailValid && password? 'bg-gradient-primary': 'bg-gradient-primary-light'}` } disabled = { !isFormValid }>
                    Login
              ...
              <ul>
                ...
                <li><GoogleSignup /></li>
                <li>
                  <p className = 'text-sm text-grey-800 text-center pt-4'>Don&apos;t have an account? <Link className = 'text-primary font-bold' to = "/signup">Sign Up</Link></p>
                </li>
              </ul>
            </div>
          </div>
